...
```{r}
ggplot(data = Pine, mapping = aes(x=datetime, y=cfs, color=cfs))+
  geom_line()+
   ylab("Discharge (cfs)")+
  xlab(element_blank())+
  theme_light()
...
```{r}
ggplot(data = SNP, mapping = aes(x=datetime, y=cfs))+
  geom_line()+
   ylab("Discharge (cfs)")+
  xlab(element_blank())+
  facet_wrap(facets = "StationID", nrow = 2)+ #ncol would be for column
  theme_light()
```
...
```{r}
ggplot(data=RBI, aes(x=RBI, y=DRAIN_SQKM,size=T_AVG_SITE, bg=STATE))+
